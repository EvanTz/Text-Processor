...
//			prefixes.add(l.get(2));
	...
	public int exportMarkDown(String outputFileName) {
		...
		MarkdownExporter exporter = new MarkdownExporter(this.document, outputFileName+"_"+alias+".md");
		int outputNumParagraphs = exporter.export();
		System.out.println("[Engine.loadProcessMarkup] [file: " + simpleInputFileName + "] exported as " + outputFileName+"_"+alias+".md"+"\n Input #pars: " + this.lineblocks.size() + " Output #pars: " + outputNumParagraphs);
	...
	public int exportPdf(String outputFileName) {
		...
		PdfExporter exporter = new PdfExporter(this.document, outputFileName+"_"+alias+".pdf");
		int outputNumParagraphs = exporter.export();
	...
	public List<String> reportWithStats(){
		...
		if(numParagraphs==0)
			loadFileAndCharacterizeBlocks();
		numParagraphs = this.lineblocks.size();
	...
	private int loadRawDocument(String fileName) {
		...
		document.setLineblocks(lineblocks); // update lineblocks in document
		...
		for(LineBlock l: lineblocks) {
			...
			for(String prefix: prefixes) {
				if(l.getLines().get(0).startsWith(prefix)) {
					String newString = l.getLines().get(0).replaceFirst(prefix, "");  //alternative way using setLines
					//l.getLines().set(0, newString); 
					l.setLines(0, newString);
				}
			}
		}
		document.setLineblocks(lineblocks); // update lineblocks in document
